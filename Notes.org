...
XIOS (which I think is what they use in CROCO-WRF).
...
All: Modified myenv_mypath.sh appropriately.
...
with the flag --netcdf_lib netcdf4_seq
...
*** WRF Compilation:
This needs to be done use the CROCO version of WRF. The changes in
configure.wrf are actually not that extensive, so it may be ok. Might
also be able to use CMS's run_compile script?

Need to include XIOS and OASIS
