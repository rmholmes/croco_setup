...
| exp05  | PP81 mixing, started from exp10end. TIW-outputF | DONE
...
| exp09  | Control E3, initialized from exp9end.TIW-outputF| DONE
...
| exp17  | exp12 E3, initialized from exp9end. TIW-outputF | DONE
| exp18  | exp13 E3, initialized from exp9end. TIW-outputF | DONE
...
TIW-outputF has updated output to include full-depth output (needed for
heat budget).
...
everything properly. Rerunning because of this...

**** Lisa's instructions 21/3/2023:

Ok, I remember your spatial Gaussian filter. So you want to perform a
reynolds decomposition on the advective terms (u*T = u'*T' + u_m*T_m +
cross terms), is that correct ?

The vertical advective term of the heat budget computed in CROCO is
using omega and not the true velocity w. So to compute the vertical
and horizontal advective terms, you will need daily u,v, temp, zeta
and omega (but you can also compute omega offline). Then you can call
the following functions:

[zr,zw] = toolsF.zlevs(h,zeta, hc, Cs_r,Cs_w)
omega = toolsF.get_omega(u,v,zr,zw,pm,pn)  # --> only if you don't have omega
TXadv,TYadv,TVadv = toolsF.get_tracer_evolution_adv2(u,v,zr,zw,pm,pn,dt_model,t,omega)

If you already saved daily u,v,temp,omega on the upper part of the
water column, you might be able to compute the advective terms, but I
am really not sure about that as the zlevs and
get_tracer_evolution_adv2 may need the entire water column as
well. Otherwise, you will need to re-run the model to save them on the
entire water column.

Just a quick note on the python-to-fortran codes to compute the heat
advection: Once you have all the daily outputs you need, and you are
ready to compute the advective terms of the heat budget, you will need
to chose either the transport form (uT) or the advective form
(u*dT/dx). I think the code "get_tracer_evolution_adv2" that I gave
you is designed to give the advective form. If you want the transport
form, you just need to comment the lines 150 to 164 (starting from "!
put in advection instead of flux : udx(tracer) instead of
dx(u*tracer)").
...
* Data removal after publishing (19/9/2024):
Input data:
- Deleted everything in /g/data/e14/rmh561/croco/data/DATA/ERA5_PAC12_75 and ../SODA342_PAC12_75 since it can all be regenerated following instructions above (search SODA 3.4.2 ini and bry processing and ERA5 forcing just below it above).
- Deleted all boundary files in /g/data/e14/rmh561/croco/CROCO_FILES/*. Kept all the initial conditions files for the ensembles.